...
export const tagWithEmptyValue = (card: Card, platform: Platform, tag: iTag) => {
    const what = Tips.what(`The <code>${tag.label}</code> tag doesn't contain any value.`)
    const why = Tips.why(`A missing value can block crawlers form properly read and process your page meta tags.`)
    const how = Tips.how(`Add the missing value to maximize the impact of every on ${platform} post about this page.`)
    card.add(
        CardBlocks.tip(
...
export const tagRepRedundantValue = (card: Card, tags: iTag[], redundantValues: string[]) => {
    ...
    const how = Tips.how(`Remove the listed values, and if nothing else is left, remove the entire meta tag.`)
    card.add(
        CardBlocks.tip(
...
export const tagUrlIsRelativePath = (card: Card, platform: Platform, tag: iTag, canonical: string) => {
    const what = Tips.what(`The Meta Tag <code>${tag.label}</code> for ${platform} is using a relative url path.`)
    const why = Tips.why(
...
export const tagUrlUsesObsoleteProtocol = (card: Card, platform: Platform, tag: iTag) => {
    ...
    const how = Tips.how(`This is the meta tag you should fix:`, CardBlocks.code(tag.code, Mode.html))
    card.add(
        CardBlocks.tip(
...
export const tagLongerThanRecommended = (
    ...
    const how = Tips.how(`Reduce the length of the tag value to ${recommendedLength.toFixed()}.`)
    card.add(
        CardBlocks.tip(
...
export const tagLongerThanMax = (card: Card, platform: Platform, tag: iTag, maxLength: number) => {
    const what = Tips.what(`The <code>${tag.label}</code> tag value is <b>${tag.value.length}</b> characters long.`)
    const why = Tips.why(
...
export const tagShouldBeSpecific = (card: Card, platform: Platform, tagLabel: string, fallbackTag: iTag) => {
    ...
    const how = Tips.how(`Add the following meta tag to your page:`, tagExample(tagLabel, platform))
    card.add(
        CardBlocks.tip(
...
export const tagIsMissing = (card: Card, platform: Platform, tagLabel: string) => {
    ...
    const how = Tips.how(`Add a meta tag similar to the following:`, tagExample(tagLabel, platform))
    card.add(
        CardBlocks.tip(
...
export const tagImageIsTooSmall = (
    ...
    const what = Tips.what(`The images is smaller that the size recommended by Twitter specs.`)
    const why = Tips.why(
...
export const tagImageIsTooLarge = (
    ...
    const what = Tips.what(`The images is larger that the size recommended by Twitter specs.`)
    const why = Tips.why(
        `Image Meta Tags must conform to the ${platform} specs in order to be successfully included in a social media post.`,
        `While they don't have a direct impact on SEO, non properly sized images can affect how the page will appear when shared on ${platform}.`
    )
