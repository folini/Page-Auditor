...
export const unsafeSitemapLinkInRobots = (urls: string[]) => {
    ...
    let message =
        ...
        codeBlock(urls.map(url => `Sitemap: ${url}`).join('\n'), Mode.txt) +
        paragraph(
    ...
    return new Card().suggestion(message, links, `Fix Sitemap Link${plural} in Robots.txt`)
}

export const repeatedSitemapLinkInRobots = (urls: string[]) => {
    const plural = urls.length > 1 ? 's' : ''
    let message =
        paragraph(`The <code>robots.txt</code> file refers multiple times the following sitemap${plural}:`) +
        codeBlock(urls.map(url => `Sitemap: ${url}`).join('\n'), Mode.txt) +
        paragraph(
    ...
    return new Card().suggestion(message, links, `Remove Duplicate Sitemap Link${plural} in Robots.txt`)
}
...
export const openGraphMissingImage = () => {
    const message =
        ...
        ) + codeBlock(`<meta property="og:image" content="https://www.example.com/image.jpg">`, Mode.html)
    const links: iLink[] = [{label: 'Facebook Meta Tags', url: 'https://ogp.me/'}]
    return new Card().suggestion(message, links, 'Add Image Meta Tag for Facebook')
}
...
export const twitterMissingImage = () => {
    const message =
        paragraph(
            `This page is missing a meta tag specifying the image for Twitter to use when rendering a post sharing this page. ` +
                `This is an example of the meta tag that should be added:`
        ) + codeBlock(`<meta name="twitter:image" content="https://www.example.com/image.jpg">`, Mode.html)
    const links: iLink[] = [
        {
    ...
    return new Card().suggestion(message, links, 'Add Image Meta Tag for Twitter')
}
...
const paragraph = (text: string) => `<div>${text}</div>`
