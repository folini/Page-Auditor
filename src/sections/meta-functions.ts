...
export const twitterPreview = (
    ...
    const twitterImg =
        tags.find(m => m.property === 'twitter:image' || m.property === 'twitter:image:src')?.content || ''
    let img =
    ...
    if (img.includes('/assets/no-image-')) {
        Tips.emptyTwitterImage(card, img)
    }

    if (twitterImg.length === 0 && img.length === 0) {
        Tips.lackingTwitterImageMetaTag(card)
    }

    if (twitterImg.length === 0 && img.length > 0) {
        Tips.lackingSpecificTwitterImageMetaTag(card)
    }

    fileExists(img).catch(() => {
        Tips.missingTwitterImage(card, img)
    })

    const twitterTagImg = tags.find(m => m.property === 'twitter:image')
    if (twitterTagImg !== undefined && !twitterTagImg.content.startsWith('http')) {
        Tips.tagWithRelativeUrlPath(card, 'twitter:image', twitterTagImg.originalCode)
    } else if (twitterTagImg !== undefined && twitterTagImg.content.startsWith('http://')) {
        Tips.tagWithUnsafeUrl(card, 'twitter:image', twitterTagImg.originalCode)
    }

    const twitterTagSrcImg = tags.find(m => m.property === 'twitter:image:src')
    if (twitterTagSrcImg !== undefined && !twitterTagSrcImg.content.startsWith('http')) {
        Tips.tagWithRelativeUrlPath(card, 'twitter:image:src', twitterTagSrcImg.originalCode)
    } else if (twitterTagSrcImg !== undefined && twitterTagSrcImg.content.startsWith('http://')) {
        Tips.tagWithUnsafeUrl(card, 'twitter:image:src', twitterTagSrcImg.originalCode)
    }

    const twitterTagUrl = tags.find(m => m.property === 'twitter:url')
    if (twitterTagUrl !== undefined && !twitterTagUrl.content.startsWith('https://')) {
        Tips.tagWithRelativeUrlPath(card, 'twitter:url', twitterTagUrl.originalCode)
    } else if (twitterTagUrl !== undefined && twitterTagUrl.content.startsWith('http://')) {
        Tips.tagWithUnsafeUrl(card, 'twitter:url', twitterTagUrl.originalCode)
    }
...
export const openGraphPreview = (
    ...
    if (img.includes('/assets/no-image-')) {
        Tips.emptyOpenGraphImage(card, img)
    }

    if (img.length === 0) {
        Tips.lackingOpenGraphImageMetaTag(card)
    }

    fileExists(img).catch(() => {
        Tips.missingOpenGraphImage(card, img)
    })

    const openGraphImgTag = tags.find(m => m.property === 'og:image')
    if (openGraphImgTag !== undefined && !openGraphImgTag.content.startsWith('http')) {
        Tips.tagWithRelativeUrlPath(card, 'og:image', openGraphImgTag.originalCode)
    } else if (openGraphImgTag !== undefined && openGraphImgTag.content.startsWith('http://')) {
        Tips.tagWithUnsafeUrl(card, 'og:image', openGraphImgTag.originalCode)
    }

    const openGraphTagUrl = tags.find(m => m.property === 'og:url')
    if (openGraphTagUrl !== undefined && !openGraphTagUrl.content.startsWith('http')) {
        Tips.tagWithRelativeUrlPath(card, 'og:url', openGraphTagUrl.originalCode)
    } else if (openGraphTagUrl !== undefined && openGraphTagUrl.content.startsWith('http://')) {
        Tips.tagWithUnsafeUrl(card, 'og:url', openGraphTagUrl.originalCode)
    }
