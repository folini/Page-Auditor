...
// © 2021 - Franco Folini
// ----------------------------------------------------------------------------
import {iMetaTag, iDefaultTags} from "./meta"

interface iTagCategoryPreview {
  (m: iMetaTag[], t: iDefaultTags): string
}
...
export interface iTagCategory {
  ...
  preview: iTagCategoryPreview
}

const noPreview: iTagCategoryPreview = (m: iMetaTag[], t: iDefaultTags) => ""

const twitterPreview = (tags: iMetaTag[], defaults: iDefaultTags): string => {
  const twitterLinkIcon =
    ...
    `</svg>`
  const title =
    tags.find(m => m.property === "twitter:title")?.content || defaults.title
  const img =
    tags.find(
      m => m.property === "twitter:image" || m.property === "twitter:image:src"
...
const openGraphPreview = (tags: iMetaTag[], defaults: iDefaultTags) => {
...
export const tagCategories: iTagCategory[] = [
  ...
  },
  {
    ...
    filter: m =>
      m.property.startsWith("cxense:") || m.property.startsWith("cxenseparse:"),
    preview: noPreview,
  },
  {
    title: `OutBrain Tags`,
