...
export const twitterTags = (card: Card, allTags: iTag[], selectedTags: iTag[], canonical: string) => {
    ...
    const imgFallbackTag = allTags.find(m => m.label === 'og:image' || m.label === 'image')
    const titleFallbackTag = allTags.find(m => m.label === 'og:title' || m.label === 'title')
    const descriptionFallbackTag = allTags.find(m => m.label === 'og:description' || m.label === 'description')
    const urlFallbackTag = allTags.find(m => m.label === 'og:url' || m.label === 'url')
    ...
    if (titleTag) {
        if (titleTag.value.length === 0) {
            ...
            Tips.tag_OverRecommendedLength(card, 'Twitter', titleTag, specs.twitterTags.twTitle.MaxLen, specs.twitterTags.twTitle.MaxRecommendedLen)
        }
    } else {
        if (title.length > 0) {
    ...
    if (descriptionTag) {
        if (descriptionTag.value.length === 0) {
            ...
            Tips.tag_OverRecommendedLength(card, 'Twitter', descriptionTag, specs.twitterTags.twDesc.MaxLen, specs.twitterTags.twDesc.MaxWithUrlLen)
        }
    } else {
